server <- function(input, output) {
  ...
  date_selected <- eventReactive(input$go_csv, {
    ...
    if (input$quick_update == TRUE) {
      ...
      if (input$quick_update_selection == '1 Week Ago') {
        aux_date <- today() - 8
      }
      if (input$quick_update_selection == '2 Weeks Ago') {
        aux_date <- today() - 15
      }
      if (input$quick_update_selection == '1 Month Ago') {
        aux_date <- today() - 31
      }
    } else{
      aux_date <- input$date
  ...
  ########################### ********** Market Index Returns #####################
  market_index_return_model <-
    eventReactive(input$go_sharpe_model, {
  ...
  output$quick_update_conf <- renderUI({
    if (input$quick_update == TRUE) {
      ...
      selectInput(
        'quick_update_selection',
        'What time period?',
        c('1 Week Ago', '2 Weeks Ago', '1 Month Ago'),
        selected = '1 Week Ago'
      )
    }
  ...
  output$market_portfolio_sr <- renderPlotly({
    # Choosing the timeframe to analyze
    if (input$period_to_analyze_sr == '1 Week Ago') {
      aux_data <- 7
    }
    if (input$period_to_analyze_sr == '2 Weeks Ago') {
      aux_data <- 15
    }
    if (input$period_to_analyze_sr == '1 Month Ago') {
      aux_data <- 31
    }
    if (input$period_to_analyze_sr == 'Whole Period') {
      aux_data <- NULL
    }
    ...
    res <- ggplot() +
      geom_path(data = port,
                aes(x = Data, y = PortSharpeRatio),
                colour = 'red') +
      geom_point(data = port,
                 aes(x = Data, y = PortSharpeRatio),
                 colour = 'red') +
      geom_path(data = market,
                aes(x = Data, y = MarketSharpeRatio),
                colour = 'blue') +
  ...
  output$market_portfolio_CAPM <- renderPlot({
    port <-
      port.pret.cr() %>% drop_na() %>% filter(Stocks == 'Portfolio') %>% rename(P.PRet = PRet)
    market <-
      market_index_return() %>% drop_na() %>% rename(M.PRet = PRet)
  ...
  output$stock_selection_risk <- renderUI({
    ...
    selectInput('stock_selection_risk_ui',
                'Which Stock do you wish to study?',
                aux,
                'Portfolio')
  ...
  output$`ljung-box` <- renderDT({
    ...
    btest <- epsilon.t %>% Box.test(type = 'Ljung-Box') %>%
      ...
      select(-parameter) %>%
      mutate(
        Interpretation = if_else(p.value > p, 'Not Serially Correlated', 'Serially Correlated'),
  ...
  output$market_portfolio_model_sr <- renderPlotly({
    # Choosing the timeframe to analyze
    if (input$period_to_analyze_model_sr == '1 Week Ago') {
      aux_data <- 7
    }
    if (input$period_to_analyze_model_sr == '2 Weeks Ago') {
      aux_data <- 15
    }
    if (input$period_to_analyze_model_sr == '1 Month Ago') {
      aux_data <- 31
    }
    if (input$period_to_analyze_model_sr == 'Whole Period') {
      aux_data <- NULL
    }
    ...
    # Portfolio M. Sharpe Ratio
    port <-
      port.pret.cr() %>% filter(Stocks == 'Portfolio') %>% drop_na()
